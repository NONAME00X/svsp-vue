<template>
  <div>
    ...
    <el-row>
      <el-col :span="8">
        <el-input v-model="search" placeholder="视频标题" style="width: 80%;"></el-input>
        <el-button @click="getData" type="success">搜索</el-button>
      </el-col>
      <el-col :span="3">
        <el-select v-model="state" placeholder="请选择" style="width: 240px" @change="changeState">
    ...
    <el-row>
      <el-table :data="videoList" style="width: 100%">
        ...
        <el-table-column label="操作" align="center" #default="scoped">
          ...
          <el-button type="success" @click="openCommitDialog(scoped.row.id)"
            v-if="scoped.row.state != 'video_pass'">审核</el-button>
        </el-table-column>
      </el-table>
    </el-row>
    ...
    <!-- 审核 -->
    <el-dialog v-model="dialogVisible" title="审核" width="500" center>
      <el-radio-group style=" display: flex;
                        flex-flow: column nowrap;
                        align-items: flex-start;" v-model="videoPass">
        <el-radio :value="true" size="large">审核通过</el-radio>
        <el-radio :value="false" size="large">审核不通过</el-radio>
      </el-radio-group>
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="dialogVisible = false">取消</el-button>
          <el-button type="primary" @click="commitState();">
            确定
          </el-button>
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      // `模板字符串`  方便拼接字符串
      this.$axios.get(`video/findAll/${this.page}/${this.size}`, {
          params: {
            search: this.search,
            state: this.state
          }
    ...
    commitState() {
      try {
        this.$axios.get(`video/commit/${this.selectVid}`, {
          params: {
            videoPass: this.videoPass
          }
        }).then(res => {
          console.log(res.data)
